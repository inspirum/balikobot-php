...
class DropPackagesTest extends AbstractBalikobotTestCase
{
    ...
    public function testInvalidRequest()
    {
        ...
        try {
            ...
            $this->assertEquals('Zásilka neexistuje, nebo již byla zpracována.', $exception->getErrors()[0]['status']);
            $this->assertTrue(isset($exception->getErrors()[1]['status']));
            $this->assertEquals('Nedorazila žádná data ke zpracování nebo nemůžou být akceptována.', $exception->getErrors()[1]['status']);
        }
    }
}
